...
open class LBXScanView: UIView {
    ...
    */
    ...
    func drawScanRect() {
        ...
        // Top left corner
        context.move(to: CGPoint(x: leftX + wAngle, y: topY))
        context.addCurve(to: CGPoint(x: (leftX - linewidthAngle / 2) + 3, y: topY + radius),
                         control1: CGPoint(x: leftX - linewidthAngle / 2, y: topY),
                         control2: CGPoint(x: (leftX - linewidthAngle / 2) + 3, y: topY))

        // Top right corner
        context.move(to: CGPoint(x: (rightX + linewidthAngle / 2) - radius, y: topY))
        context.addCurve(to: CGPoint(x: (rightX + linewidthAngle / 2) - 3, y: topY + radius),
                         control1: CGPoint(x: rightX + linewidthAngle / 2, y: topY),
                         control2: CGPoint(x: (rightX + linewidthAngle / 2) - 3, y: topY))


        // Bottom right corner
        context.move(to: CGPoint(x: rightX, y: (bottomY + linewidthAngle / 2) - radius))
        context.addCurve(to: CGPoint(x: rightX - radius, y: (bottomY + linewidthAngle / 2) - 3),
                         control1: CGPoint(x: rightX, y: (bottomY + linewidthAngle / 2)),
                         control2: CGPoint(x: rightX, y: (bottomY + linewidthAngle / 2) - 3))

        // Bottom left corner
        context.move(to: CGPoint(x: leftX, y: (bottomY + linewidthAngle / 2) - radius))
        context.addCurve(to: CGPoint(x: leftX + radius, y: (bottomY + linewidthAngle / 2) - 3),
                         control1: CGPoint(x: leftX, y: (bottomY + linewidthAngle / 2)),
                         control2: CGPoint(x: leftX, y: (bottomY + linewidthAngle / 2) - 3))
